...
func IsPresent() (bool, error) {
	...
	// The MinIO image always sets the MINIO_KMS_SECRET_KEY_FILE env. var.
	// Hence, it would not be possible to use MinKMS or KES if we are very
	// strict about it. This needs to be fixed in the container image. Until
	// then, we relax this check.
	staticKeyPresent := isPresent(EnvKMSSecretKey, EnvKMSSecretKeyFile) && (os.Getenv(EnvKMSSecretKeyFile) != "" || os.Getenv(EnvKMSSecretKey) != "")
