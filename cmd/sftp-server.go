...
func startSFTPServer(args []string) {
	...
	if userCaKeyFile != "" {
		...
		if err != nil {
			logger.Fatal(fmt.Errorf("invalid arguments passed, trusted user CA public key file is not accessible: %v", err), "unable to start SFTP server")
		}

		caPublicKey, _, _, _, err := ssh.ParseAuthorizedKey(keyBytes)
		if err != nil {
			logger.Fatal(fmt.Errorf("invalid arguments passed, trusted user CA public key file is not parseable: %v", err), "unable to start SFTP server")
		}
